...
bar8_tiv_np = np.load('data/bar8_cp_tiv_np.npy')
bar8_cp_np = np.load('data/bar8_cp_np.npy')

bar8_comb = []
for i in range(bar8_cp_np.shape[0]):
    bar8_comb.append([bar8_tiv_np[i], bar8_cp_np[i]])

bar8_cp_tensor = torch.tensor(bar8_comb, dtype=torch.float32)
...
for epoch in range(NUM_EPOCHS):
    ...
    if epoch % 10 == 0:
        ...
        print('train: ', test_recon(bar8_cp_tensor[:6000,0,:,:], bar8_cp_tensor[:6000,1,:,:]))
        print('test: ', test_recon(bar8_cp_tensor[6000:,0,:,:], bar8_cp_tensor[6000:,1,:,:]))
        model.train()